...
// split data by /##\|\|##\n{2}/ to get eachEmails
// map through all emails to get date objects
  // split eachEmail by /#/#\n/ to seperate different parts of eachEmail
  // email[2] to get the date string
    // parse date string => create date object new Date(datestr)
// find out min and max date objects
  // sort them


// > a = new Date()
// 2019-06-05T08:46:21.214Z
// > b = new Date()
// 2019-06-05T08:46:25.791Z
// > a - b
// -4577
// >

// a function to split emails
// a function to return an Date object when pass in a single email
// a function to sort date objects

let emails = emailData.split(/##\|\|##\n{2}/);

let dateObjects = emailData.match(/\d{2}-\d{2}-\d{4}/g).map(dateStr => {
  return new Date(dateStr);
})

let sortedDates = dateObjects.sort((a, b) => a - b);
let formattedDates = sortedDates.map(date => date.toDateString());

console.log(`Count of Email: ${emails.length}`);
console.log(`Date Range: ${formattedDates[0]} - ${formattedDates[formattedDates.length - 1]}`);
